function varargout = interface_ajuste_wasige2019_1(varargin)
...
% varargin   command line arguments to interface_ajuste (see VARARGIN)
     ...
     % *Dados da Referência
     S = load('tensao_ref_wasige2019.txt');                    %Carrega os dados do .txt. MELHORA: botao para carregar o arquivo a partir da interface
     handles.tensao_referencia = S;                         %Salva os dados no objeto tensao_referencia
     S = load('corrente_ref_wasige2019.txt');                  %Carrega os dados do .txt. MELHORA: botao para carregar o arquivo a partir da interface
     ...
     Er = 0.1253;        %Parâmetro de Energia da Primeira Ressonância salvo no objeto Er
     gamma = 0.0194;     %Parâmetro da Largura da Primeira Ressonância salvo no objeto gamma
...
% *Carregamento dos parâmetros da etapa anterior
     A = 1662700000;      %Parâmetro A
     B = 0.0585;       %Parâmetro B
...
% * Carregamento dos parâmetros da referência     
     V_tensao_referencia = load('tensao_ref_wasige2019.txt');     %Carrega um vetor com a tensão da referência
     J_referencia = load('corrente_ref_wasige2019.txt');          %Carrega um vetor com a corrente da referência
...
% * Simulação da Densidade de Corrente pelo Diodo de Tunelamento Ressonante     
     ...
     plot(V_tensao_referencia,J_referencia,V_tensao,Ima);
     legend('Referência','Simulacao','Location','northeast')
     
     % ** PVR da curva simulada inicial
     corrente_pico = findpeaks(J);                          %Encontra a corrente de máximo
     corrente_vale = -findpeaks(-J);                        %Encontra a corrente de mínimo
     handles.PVR_simulacao0 = corrente_pico/corrente_vale;         %Salva a razão pico-vale de corrente no objeto PVR_simulacao0
     set(handles.txtPVRsimu, 'String', handles.PVR_simulacao0);     %Ajusta a mensagem do objeto txtPVRref
