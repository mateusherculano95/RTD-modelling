%-------------------------------Descrição----------------------------------
% Engine para simular a curva do dispositivo construído em Wasige et al 
% (2019).
%--------------------------------------------------------------------------
...
% Contato ohmico superior % -         % -             % Ti/Pd/Au
% Espaçador 1             % 400 A     % 3e19 cm-3     % In0,53Ga0,47As
% Espaçador 2             % 1600 A    % 2e18 cm-3     % In0,53Ga0,47As
% Espaçador 3             % 250 A     % 2e16 cm-3     % In0,53Ga0,47As
% Emissor                 % 15 A      % alta          % In0,53Ga0,47As
% Barreira                % 14 A      % intrínseco    % AlAs
% Poço quântico           % 45 A      % intrínseco    % In0,53Ga0,47As
% Barreira                % 14 A      % intrínseco    % AlAs
% Coletor                 % 15 A      % alta          % In0,53Ga0,47As
% Espaçador 3             % 250 A     % 2e16 cm-3     % In0,53Ga0,47As
% Espaçador 2             % 250 A     % 2e18 cm-3     % In0,53Ga0,47As
% Espaçador 4             % 100 A     % -             % In0,53Ga0,47As
% Espaçador 1             % 4000 A    % 3e19 cm-3     % In0,53Ga0,47As
% Contato ohmico inferior % -         % -             % Ti/Pd/Au
%--------------------------------------------------------------------------
...
% **DBQW
Lb                   = 14;              %Comprimento da Barreira [A]
Lw                   = 45;              %Comprimento do Poço [A]
mb                   = 0.146*m0;         %Massa efetiva da barreira [kg]
mw                   = 0.041*m0;        %Massa efetiva do poço [kg]
V0                   = 0.658144;               %Altura do potencial [eV]
% **Emissor
Nd                   = 2e18;            %Dopagem do Emissor [cm-3]
me                   = 0.041*m0;        %Massa efetiva do elétron no reservatório [kg]   
mh                   = 0.45*m0;         %Massa efetiva da lacuna no reservatório [kg]
Eg                   = 0.74;           %Energia de gap no reservatório [eV]
Ef                   = 0;               %Energia de Fermi no reservatório [eV]
Dn                   = 300;             %Coeficiente de difusão dos elétrons [cm2/s]
Dp                   = 7.2;             %Coeficiente de difusão das lacunas [cm2/s]
Ln                   = 2.5;           %Comprimento de difusão de elétrons [um]
Lp                   = 100;           %Comprimento de difusão de lacunas [um]


% *Parâmetros de polarização
V_max                = 2;             %Tensão máxima da amostra [V]
...
[H]            = 1e4*parametro_H(Dn, Na, Ln, Dp, Nd, Lp, ni);                     %calcula o parâmetro H [A/cm2]
...
[A]            = criaescalar_A(mw,gamma);                                     %calcula o parâmetro A [A/cm2]
B              = Ef;
...
[ET,IT, ImaT, JT, mT, Trans_logT, V_tensaoT, zT, VT] = save_origin_3(E, I, Ima, J, m, Trans_log, V_tensao, z, V);
%--------------------------------------------------------------------------
