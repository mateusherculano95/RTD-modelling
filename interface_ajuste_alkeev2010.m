function varargout = interface_ajuste_alkeev2010(varargin)
...
% varargin   command line arguments to interface_ajuste (see VARARGIN)
     ...
     % *Dados da Referência
     S = load('tensao_ref_alkeev2010.txt');                    %Carrega os dados do .txt. MELHORA: botao para carregar o arquivo a partir da interface
     handles.tensao_referencia = S;                         %Salva os dados no objeto tensao_referencia
     S = load('corrente_ref_alkeev2010.txt');                  %Carrega os dados do .txt. MELHORA: botao para carregar o arquivo a partir da interface
     ...
     S = load('tensao_simu_alkeev2010.txt');
     handles.tensao_simulacao0 = S;
     S = load('corrente_simu_alkeev2010.txt');
     ...
     Er = 0.1726;        %Parâmetro de Energia da Primeira Ressonância salvo no objeto Er
     gamma = 0.0022;     %Parâmetro da Largura da Primeira Ressonância salvo no objeto gamma
...
% *Carregamento dos parâmetros da etapa anterior
     A = 314470000;      %Parâmetro A
     B = 0.0394;       %Parâmetro B
     H = 0.069851;     %Parâmetro H
...
% * Carregamento dos parâmetros da referência     
     V_tensao_referencia = load('tensao_ref_alkeev2010.txt');     %Carrega um vetor com a tensão da referência
     J_referencia = load('corrente_ref_alkeev2010.txt');          %Carrega um vetor com a corrente da referência
